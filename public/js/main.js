...
socket.on("turnTimesLeft", function (data) {
	...
	for (let i = 0; i < data.turnTimesLeft.length; i++) {
		...
		let percent = parseInt((data.turnTimesLeft[i] / data.turnLengths[i]) * 100);

		let timeLeftSec2 = parseInt(data.forfeitTimesLeft[i] / 1000);
		let percent2 = parseInt((data.forfeitTimesLeft[i] / data.forfeitLengths[i]) * 100);

		let n = i + 1;

		ReactDOM.render(<TurnTimer name={usernameMap[controlQueues[i][0]]} percent={percent} timeLeft={timeLeftSec}/>, document.getElementById("turnTimerBar" + n));
		ReactDOM.render(<ForfeitTimer name={usernameMap[controlQueues[i][0]]} percent={percent2} timeLeft={timeLeftSec2}/>, document.getElementById("forfeitTimerBar" + n));
