...
self.addEventListener("message", function(e) {
	...
	let data = e.data;
	let inc = data.inc; //true = data is from the server | false = data comeing from client mic
	let inDataArrayBuffer = data.inDataArrayBuffer; //Incoming Data
	let inSampleRate = data.inSampleRate;
	let outSampleRate = data.outSampleRate;
	let inBitrate = data.inBitRate;
	let outBitRate = data.outBitRate;
	let outChunkSize = data.outChunkSize;
	let minGain = data.minGain;
	let clientId = data.socketId;
	let p = data.p;
	...
	if (inc) { //Data are from an other client
		...
		if (inDataArrayBuffer != null) {
			resample(inDataArrayBuffer, inSampleRate, outSampleRate, outChunkSize, function(resapledData) {
				...
				let lowDat = lowSignal(resapledData, p);
...
function mapFloat32ToUInt16Array(flt32) {
	let uint16Array = new Uint16Array(flt32.length);
	for (let i = 0; i < flt32.length; i++) {
...
function mapUint16ToFloat32Array(uin16) {
	let flt32Array = new Float32Array(uin16.length);
	for (let i = 0; i < uin16.length; i++) {
...
function mapUint8ToFloat32Array(uin8) {
	let flt32Array = new Float32Array(uin8.length);
	for (let i = 0; i < uin8.length; i++) {
...
function mapFloat32ToUInt8Array(flt32) {
	let uint8Array = new Uint8Array(flt32.length);
	for (let i = 0; i < flt32.length; i++) {
...
(function() {
	let AbstractInterpolator, CubicInterpolator, Enum, LinearInterpolator, NearestInterpolator, PI, SincFilterInterpolator, Smooth, clipClamp, clipMirror, clipPeriodic, defaultConfig, getColumn, getType, isValidNumber, k, makeLanczosWindow, makeScaledFunction, makeSincKernel, normalizeScaleTo, shallowCopy, sin, sinc, v, validateNumber, validateVector,
		...
		smoothFunc = (function() {
			...
			switch (getType(arr[0])) {
				...
				case "Array":
					...
					return function(t) {
						let interpolator, _k, _len3, _results;
						_results = [];
