...
export class MPEG2 extends BaseDecoder {

    /**
     * MPEG2 декодер
     * винмание на стр 187 - 189
     * @param options
     * @constructor
     */
    constructor (options) {
        ...
        var bufferMode = options.streaming
            ? BitBuffer2.MODE.EVICT
            : BitBuffer2.MODE.EXPAND;

        this.bits = new BitBuffer2(bufferSize, bufferMode);
		...
		/**
		 * Тип фрейма
		 * 000 forbidden 001 intra-coded (I) 010 predictive-coded (P) 011 bidirectionally-predictive-coded (B)
		 * @type {number}
		 */
		this.pictureType = 0;
		...
		/**
		 * Часть спеки MPEG1 не используется в MPEG2 должно быть равно 0х0
		 * @type {boolean}
		 */
		this.fullPelForward = false;

		/**
		 * Часть спеки MPEG1 не используется в MPEG2 должно быть равно 0х111
		 * @type {number}
		 */
		this.forwardFCode = 0;
		...
		/**
		 * Признак того что буфере индекс находится в позиции данных слайса
		 * @type {boolean}
		 */
		...
		/**
		 * Адрес текущего макроблока
		 * @type {number}
		 */
		this.macroblockAddress = 0;

		/**
		 * Строка макроблока в структуре фрейма
		 * @type {number}
		 */
		this.mbRow = 0;

		/**
		 * Колонка макроблока в структуре фрейма
		 * @type {number}
		 */
		this.mbCol = 0;
		...
		/**
		 * ? Признак того что весь макроблок I ?
		 * @type {boolean}
		 */
		this.macroblockIntra = false;

		/**
		 * ? Признак того что макроблок лолжен использовать компенсацию движения ?
		 * @type {boolean}
		 */
    ...
    /**
     * Записать данные в буфер сохранить pts для них
     * @param pts
     * @param buffers
     * @returns {boolean}
     */
    write = function (pts, buffers) {
        ...
        if (!this.hasSequenceHeader) {
            //найдем его начало
            if (this.bits.findStartCode(MPEG2.START.SEQUENCE_HEADER) === -1) {
                return false;
            }
            //разберем залоговок
            this.decodeSequenceHeader();
            //раскодировать один фрейм
    ...
    decode = () => {
        ...
        //вызвать колбек после декодинга, если он есть
    ...
    decodeSequenceHeader = function () {
        ...
        // skip pixel aspect ratio (стр 47)
        this.bits.skip(4);

        //узнаем framerate (стр 48)
        this.frameRate = MPEG2.PICTURE_RATE[this.bits.read(4)];

        // skip bitRate, marker, bufferSize and constrained bit (стр 49)
        this.bits.skip(18 + 1 + 10 + 1);

        /**
         * видимо по умолчанию у нас будет undefined т.ч. установим ширину и высоту, проинициализируем буферы
         * и установим размер плеера
         */
        ...
        /**
         * если есть признак того, что матрицаы квантования кастомные, нужно её загрузить (стр 49)
         * load_intra_quantiser_matrix -- See 6.3.11 “Quant matrix extension”
         * порядок загрузки описан на (стр 81)
         */
        if (this.bits.read(1)) { // load custom intra quant matrix?
            for (let index = 0; index < 64; index++) {
                //TODO: тот ли зиг-заг?
                const zigzagElement = MPEG2.ZIG_ZAG[index];
                this.customIntraQuantMatrix[zigzagElement] = this.bits.read(8);
            }
        ...
        if (this.bits.read(1)) { // load custom non intra quant matrix?
            for (let index = 0; index < 64; index++) {
                //TODO: тот ли зиг-заг?
                const zigzagElement = MPEG2.ZIG_ZAG[index];
                this.customNonIntraQuantMatrix[zigzagElement] = this.bits.read(8);
            }
    ...
    /**
     * Инициализация буферов декодера и метаданных
     */
    initBuffers = function () {
        ...
        /**
         * размеры макроблоков (стр 47)
         * In the case that progressive_sequence is ‘1’ the height of the encoded luminance component of frames in
         * macroblocks, mb_height, is (vertical_size + 15)/16.
         */
    ...
    decodePicture = function (skipOutput) {
        ...
        if (this.pictureType === MPEG2.PICTURE_TYPE.PREDICTIVE) {
            ...
            this.bits.skip(4);
            this.fullPelForward = 0x0;
            this.forwardFCode = 0x7;
            /*
        ...
        /**
         * Обработка данных Slice (стр 70)
         * slice_start_code -- The slice_start_code is a string of 32-bits.
         * The first 24-bits have the value 000001 in hexadecimal and the last 8-bits are the slice_vertical_position
         * having a value in the range 01 through AF hexadecimal inclusive.
         */
        while (code >= MPEG2.START.SLICE_FIRST && code <= MPEG2.START.SLICE_LAST) {
            //TODO: не понятно нахуя нужно & 0x000000FF может чтобы знак выровнять? хер знает в общем
    ...
    /**
     * Раскодировать slice, сначала прочитать заголовок и потом разобрать макроблоки
     * @param slice - вертикальная позиция слайса (slice_vertical_position)
     */
    decodeSlice = function (slice) {
        this.sliceBegin = true;
        const mbRow = slice - 1;
        this.macroblockAddress = mbRow * this.mbWidth - 1;
        ...
        //TODO: есть подозрение что тут должен идти priority_breakpoint в оригинальной реализации этого нет
        /**
         *  А 7-bit integer that indicates the point in the syntax where the bitstream shall be partitioned
         */
        // this.bits.skip(7);

        /**
         * A 5 bit unsigned integer in the range 1 to 31. The decoder shall use this value until another
         * quantiser_scale_code is encountered either in slice() or macroblock(). The value  zero  isforbidden.
         * @type {number|Promise<ReadableStreamReadResult<number>>|Promise<ReadableStreamReadResult<any>>}
         */
        this.quantizerScale = this.bits.read(5);

        // skip extra bits intra_slice_flag / intra_slice / reserved_bits / extra_bit_slice / extra_information_slice
        while (this.bits.read(1)) {
            this.bits.skip(8);
        }

        /**
         * Разобрать данные макроблоков, пока не закончатся старт коды
         */
        do {
    ...
    /**
     * Разобрать данные макроблока (стр 71)
     */
    decodeMacroblock = function () {
        ...
        var cbp = ((this.macroblockType & 0x02) !== 0)
            ? this.readHuffman(MPEG2.CODE_BLOCK_PATTERN)
    ...
    decodeMotionVectors = function () {
        ...
        } else if (this.pictureType === MPEG2.PICTURE_TYPE.PREDICTIVE) {
    ...
    decodeBlock = function (block) {
        ...
        while (true) {
            var
                run = 0,
                coeff = this.readHuffman(MPEG2.DCT_COEFF);
            ...
            n += run;
            var dezigZagged = MPEG2.ZIG_ZAG[n];
            n++;
    ...
    /**
     * Таблица соответствия framerate - frame rate code из Sequence header
     * @type {number[]}
     */
    static PICTURE_RATE = [
    ...
    /**
     * Матрица зиг-заг по умолчанию Definition of scan[0][v][u] (стр 81)
     * @type {Uint8Array}
     */
    static ZIG_ZAG_DEFAULT = new Uint8Array([
        0, 1, 5, 6, 14, 15, 27, 28,
    ...
    /**
     * Матрица зиг-заг альтернативная Definition of scan[1][v][u] (стр 81)
     * @type {Uint8Array}
     */
    static ZIG_ZAG_ALTERNATE = new Uint8Array([
    ...
    /**
     * Матрица квантования по умолчанию для intra блоков (стр 65)
     * @type {Uint8Array}
     */
    ...
    /**
     * Матрица квантования по умолчанию для не intra блоков (стр 65)
     * @type {Uint8Array}
     */
    ...
    /**
     * Variable length code tables (стр 153)
     * @type {Int16Array}
     */
