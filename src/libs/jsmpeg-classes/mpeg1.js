...
export class MPEG1 extends BaseDecoder {
	constructor(options) {
		...
		this.DEFAULT_INTRA_QUANT_MATRIX = new Uint8Array([
		...
		this.DEFAULT_NON_INTRA_QUANT_MATRIX = new Uint8Array([
		...
		this.PREMULTIPLIER_MATRIX = new Uint8Array([
		...
		this.MACROBLOCK_ADDRESS_INCREMENT = new Int16Array([
			1*3,  2*3,  0, //   0
			3*3,  4*3,  0, //   1  0
			0,    0,  1, //   2  1.
			5*3,  6*3,  0, //   3  00
		...
		this.MACROBLOCK_TYPE_INTRA = new Int8Array([
			1*3,  2*3,     0, //   0
			-1,  3*3,     0, //   1  0
			0,    0,  0x01, //   2  1.
			0,    0,  0x11  //   3  01.
		...
		this.MACROBLOCK_TYPE_PREDICTIVE = new Int8Array([
			1*3,  2*3,     0, //  0
			3*3,  4*3,     0, //  1  0
			0,    0,  0x0a, //  2  1.
			5*3,  6*3,     0, //  3  00
		...
		this.MACROBLOCK_TYPE_B = new Int8Array([
			1*3,  2*3,     0,  //  0
			3*3,  5*3,     0,  //  1  0
			4*3,  6*3,     0,  //  2  1
			8*3,  7*3,     0,  //  3  00
		...
		this.MACROBLOCK_TYPE = [
			null,
			this.MACROBLOCK_TYPE_INTRA,
			this.MACROBLOCK_TYPE_PREDICTIVE,
			this.MACROBLOCK_TYPE_B
		];

		this.CODE_BLOCK_PATTERN = new Int16Array([
			2*3,   1*3,   0,  //   0
			3*3,   6*3,   0,  //   1  1
			4*3,   5*3,   0,  //   2  0
			8*3,  11*3,   0,  //   3  10
		...
		this.DCT_DC_SIZE_LUMINANCE = new Int8Array([
			2*3,   1*3, 0,  //   0
			6*3,   5*3, 0,  //   1  1
			3*3,   4*3, 0,  //   2  0
				0,     0, 1,  //   3  00.
		...
		this.DCT_DC_SIZE_CHROMINANCE = new Int8Array([
			2*3,   1*3, 0,  //   0
			4*3,   3*3, 0,  //   1  1
			6*3,   5*3, 0,  //   2  0
			8*3,   7*3, 0,  //   3  11
	...
	write = function(pts, buffers) {
		...
		if (!this.hasSequenceHeader) {
			if (this.bits.findStartCode(this.START.SEQUENCE) === -1) {
				return false;
			}
	...
	decode = function() {
		...
		if (this.bits.findStartCode(this.START.PICTURE) === -1) {
	...
	decodeSequenceHeader = function() {
		...
		this.frameRate = this.PICTURE_RATE[this.bits.read(4)];
		...
		if (this.bits.read(1)) { // load custom intra quant matrix?
			for (var i = 0; i < 64; i++) {
				this.customIntraQuantMatrix[this.ZIG_ZAG[i]] = this.bits.read(8);
			}
	...
	initBuffers = function() {
		this.intraQuantMatrix = this.DEFAULT_INTRA_QUANT_MATRIX;
		this.nonIntraQuantMatrix = this.DEFAULT_NON_INTRA_QUANT_MATRIX;
	...
	decodePicture = function(skipOutput) {
		...
		if (this.pictureType <= 0 || this.pictureType >= this.PICTURE_TYPE.B) {
			return;
		}

		// full_pel_forward, forward_f_code
		if (this.pictureType === this.PICTURE_TYPE.PREDICTIVE) {
		...
		} while (code === this.START.EXTENSION || code === this.START.USER_DATA );


		while (code >= this.START.SLICE_FIRST && code <= this.START.SLICE_LAST) {
		...
		if (
			this.pictureType === this.PICTURE_TYPE.INTRA ||
			this.pictureType === this.PICTURE_TYPE.PREDICTIVE
		) {
			var
				tmpY = this.forwardY,
	...
	decodeMacroblock = function() {
		...
		var
			increment = 0,
			t = this.readHuffman(this.MACROBLOCK_ADDRESS_INCREMENT);
		...
		var mbTable = this.MACROBLOCK_TYPE[this.pictureType];
		...
		var cbp = ((this.macroblockType & 0x02) !== 0)
			? this.readHuffman(this.CODE_BLOCK_PATTERN)
	...
	decodeMotionVectors = function() {
		...
		if (this.macroblockMotFw) {
			// Horizontal forward
			code = this.readHuffman(this.MOTION);
		...
		else if (this.pictureType === this.PICTURE_TYPE.PREDICTIVE) {
	...
	decodeBlock = function(block) {
		...
		if (this.macroblockIntra) {
			...
			if (block < 4) {
				...
				dctSize = this.readHuffman(this.DCT_DC_SIZE_LUMINANCE);
			}
			else {
				predictor = (block === 4 ? this.dcPredictorCr : this.dcPredictorCb);
				dctSize = this.readHuffman(this.DCT_DC_SIZE_CHROMINANCE);
			}
		...
		while (true) {
			var
				run = 0,
				coeff = this.readHuffman(this.DCT_COEFF);
			...
			n += run;
			if (n < 0 || n >= this.ZIG_ZAG.length) break;
			var dezigZagged = this.ZIG_ZAG[n];
			n++;
			...
			this.blockData[dezigZagged] = level * this.PREMULTIPLIER_MATRIX[dezigZagged];
		}
