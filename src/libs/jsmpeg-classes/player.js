...
export class Player {
	constructor(url, options) {
		...
		// todo: web assembly disabled until I figure out how to include it:
		if (/*!options.disableWebAssembly*/ false && WASM.IsSupported()) {
			// this.wasmModule = WASM.GetModule();
			this.wasmModule = new WASM();
		...
		if (this.wasmModule) {
			if (this.wasmModule.ready) {
				...
				var wasm = Base64ToArrayBuffer(JSMpeg.WASM_BINARY_INLINED);
